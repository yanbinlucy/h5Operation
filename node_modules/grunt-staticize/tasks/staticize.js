...
module.exports = function(grunt) {
    ...
    grunt.registerMultiTask('staticize', 'The best Grunt plugin ever.', function() {
        ...
        function repRun(files, patterns, assetsDirs) {
            ...
            //
            var fileObj = {};
            var filePathReg = new RegExp('(\\.\\w{' + (revOption.length || 8) + '})+(?=\\.\\w+)');
            allFiles.forEach(function(file) {
                var beforeHash = file.replace(filePathReg, '');
                fileObj[path.resolve(beforeHash)] = path.resolve(file);
            });
            //
            mapping.forEach(function(file) {
                ...
                var result = grunt.file.read(file).replace(patterns, function() {
                    ...
                    if (fileObj[absoPath]) {
                        ...
                        var resultPath = path.join(parentPath, path.basename(fileObj[absoPath]));
                        if (arguments[0] != resultPath) {
